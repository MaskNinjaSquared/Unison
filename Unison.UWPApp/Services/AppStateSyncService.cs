...
namespace Unison.UWPApp.Services
{
    internal sealed class AppStateSyncService
    {
        ...
        {
            var id = node?.Attrs?.GetValueOrDefault("id");
            if (string.IsNullOrWhiteSpace(id))
            {
                return;
            }

            string error = node.Attrs.GetValueOrDefault("error");
        ...
        private async Task<Dictionary<string, CollectionChunk>> QueryCollectionsAsync(IEnumerable<string> collections, HashSet<string> forceSnapshotCollections = null)
        {
            ...
            {
                string name = collectionNode.Attrs?.GetValueOrDefault("name");
                ...
                {
                    Name = name,
                    HasMorePatches = string.Equals(collectionNode.Attrs?.GetValueOrDefault("has_more_patches"), "true", StringComparison.OrdinalIgnoreCase)
                };
                ...
                foreach (var patchNode in (patchesNode ?? collectionNode).GetChildren("patch"))
                {
                    ...
                    if (patch.Version == null)
                    {
                        ...
                        long.TryParse(collectionNode.Attrs?.GetValueOrDefault("version") ?? "0", out currentVersion);
        ...
        private async Task<bool> ApplyMutationAsync(string collectionName, DecodedMutation mutation, bool isInitialSync)
        {
            ...
            {
                await _owner.ApplyAppStateChatFlagsAsync(chatJid, pinned: value.PinAction.Pinned);
                return true;
            }
