...
namespace Unison.UWPApp.Protocol
{
    ...
    public class BinaryEncoder
    {
        ...
        private void WriteString(string str)
        {
            ...
            // Check if it's a JID
            if (str.IndexOf('@') >= 0 &&
                WA.TryDecodeJid(str, out var jidUser, out var jidServer, out var jidDevice, out var jidDomainType))
            {
                if (jidDevice.HasValue)
                {
                    WriteByte(247); // AD_JID
                    WriteByte((byte)jidDomainType);
                    WriteByte((byte)jidDevice.Value);
                    WriteString(jidUser);
                }
                else
                {
                    WriteByte(250); // JID_PAIR
                    if (string.IsNullOrEmpty(jidUser))
                    {
