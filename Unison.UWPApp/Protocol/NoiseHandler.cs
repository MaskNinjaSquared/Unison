...
namespace Unison.UWPApp.Protocol
{
    ...
    public class NoiseHandler
    {
        ...
        public NoiseHandler(KeyPair keyPair, byte[] routingInfo = null)
        {
            ...
            WhatsAppService.Log($"[Noise] Initialized with public key: {BitConverter.ToString(_keyPair.Public).Replace("-", "").Substring(0, 16)}...");
        }
        ...
        public void Authenticate(byte[] data)
        {
            lock (_stateLock)
            {
                if (!_isFinished)
                {
                    ...
                    WhatsAppService.Log($"[Noise] Authenticated {data.Length} bytes, hash: {BitConverter.ToString(_hash).Replace("-", "").Substring(0, 16)}...");
                }
            }
        }
        ...
        public byte[] Encrypt(byte[] plaintext)
        {
            lock (_stateLock)
            {
                ...
                WhatsAppService.Log($"[Noise] Encrypted {plaintext.Length} bytes -> {result.Length} bytes, counter: {_writeCounter}");
                return result;
            }
        }
        ...
        public byte[] Decrypt(byte[] ciphertext)
        {
            lock (_stateLock)
            {
                ...
                WhatsAppService.Log($"[Noise] Decrypted {ciphertext.Length} bytes -> {result.Length} bytes");
                return result;
            }
        }
        ...
        public void MixIntoKey(byte[] data)
        {
            lock (_stateLock)
            {
                ...
                _readCounter = 0;
                _writeCounter = 0;
                WhatsAppService.Log($"[Noise] Mixed key, new encKey: {BitConverter.ToString(_encKey).Replace("-", "").Substring(0, 16)}...");
            }
        }
        ...
        public void FinishInit()
        {
            lock (_stateLock)
            {
                ...
                _writeCounter = 0;
                _isFinished = true;
                WhatsAppService.Log("[Noise] Handshake finished, encrypted channel established");
            }
        }
        ...
        {
            WhatsAppService.Log($"[Noise] Processing handshake...");
            WhatsAppService.Log($"[Noise] Server ephemeral: {serverEphemeral.Length} bytes");
            WhatsAppService.Log($"[Noise] Server static encrypted: {serverStaticEncrypted.Length} bytes");
            WhatsAppService.Log($"[Noise] Server payload encrypted: {serverPayloadEncrypted.Length} bytes");
            ...
            WhatsAppService.Log($"[Noise] Decrypted server static: {serverStatic.Length} bytes");
            ...
            WhatsAppService.Log($"[Noise] Decrypted cert payload: {certPayload.Length} bytes");
            ...
            WhatsAppService.Log($"[Noise] Encrypted noise key: {keyEnc.Length} bytes");
        ...
        public byte[] EncodeFrame(byte[] data)
        {
            ...
            WhatsAppService.Log($"[Noise] Encoded frame: {frame.Length} bytes (data: {data.Length}, intro: {introSize})");
            return frame;
        }
        ...
        public async Task DecodeFrame(byte[] newData, Func<byte[], Task> onFrame)
        {
            if (newData != null)
            {
                lock (_stateLock)
                {
                    ...
                    _inBytes = combined;
                    WhatsAppService.Log($"[Noise] Received {newData.Length} bytes, total buffer: {_inBytes.Length} bytes");
                }
            }
            ...
            while (true)
            {
                ...
                lock (_stateLock)
                {
                    ...
                    {
                        WhatsAppService.Log($"[Noise] Waiting for more data, need {size + 3}, have {_inBytes.Length}");
                        return;
                    }
                ...
                {
                    WhatsAppService.Log($"[Noise] Decoded frame: {frameToProcess.Length} bytes");
