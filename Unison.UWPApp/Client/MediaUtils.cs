...
namespace Unison.UWPApp.Client
{
    public static class MediaUtils
    {
        ...
        public static byte[] DecryptMedia(byte[] encryptedBytes, byte[] mediaKey, string mediaType, byte[] expectedFileEncSha256 = null)
        {
            ...
            Array.Copy(encryptedBytes, 0, ciphertext, 0, cipherLen);
            Array.Copy(encryptedBytes, cipherLen, msgMac, 0, 10);

            // mac = HMAC-SHA256(iv || ciphertext, macKey).slice(0,10)
            var ivPlusBody = new byte[keys.IV.Length + ciphertext.Length];
            Array.Copy(keys.IV, 0, ivPlusBody, 0, keys.IV.Length);
            Array.Copy(ciphertext, 0, ivPlusBody, keys.IV.Length, ciphertext.Length);
            var fullMac = CryptoUtils.HmacSha256(ivPlusBody, keys.MacKey);
            ...
            return CryptoUtils.AesCbcDecrypt(ciphertext, keys.CipherKey, keys.IV);
        }
