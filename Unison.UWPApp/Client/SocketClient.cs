    ...
    public class SocketClient : IDisposable
    {
        ...
        public async System.Threading.Tasks.Task InitializeKeyStoreAsync()
        {
            ...
            if (storedAccount != null && _authState.Account == null)
            {
                ...
                Debug.WriteLine("[Socket] Loaded account from KeyStore");
            }
        ...
        private async Task<string> SendProtoMessageAsync(string jid, Proto.Message message)
        {
            ...
            // Save sessions after encryption updates (fire and forget)
            _ = Task.Run(async () =>
            {
                foreach (var deviceJid in allTargetDevices)
                {
    ...
    System.Collections.Generic.List<BinaryNode> queryProtocols)
{
    ...
    return await QueryAsync(new BinaryNode("iq", new System.Collections.Generic.Dictionary<string, string>
    {
        ...
        private async void HandleIncomingInfo(BinaryNode node)
        {
            ...
            {
                Debug.WriteLine("[Socket] Received offline_preview - requesting offline batch");
                try
                {
                    // Send offline_batch request to receive pending messages
                    ...
                    Debug.WriteLine("[Socket] Sent offline_batch request");
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"[Socket] Failed to send offline_batch: {ex.Message}");
                }
            }
            ...
            if (offlineNode != null)
            {
                ...
                Debug.WriteLine($"[Socket] Received offline completion signal - handled {offlineCount} offline messages");
                
                // Signal that we've received all pending notifications
                // This is the "you're good to go" signal from WhatsApp
                OnReceivedPendingNotifications?.Invoke(this, offlineCount);
            }
            ...
            if (dirtyNode != null)
            {
                ...
                if (dirtyType == "account_sync" && !string.IsNullOrEmpty(timestamp))
                {
                    ...
                    _ = SendCleanDirtyBitsAsync(dirtyType, timestamp);
                }
            }
        ...
        public event EventHandler<int> OnReceivedPendingNotifications;
