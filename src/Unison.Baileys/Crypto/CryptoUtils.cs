...
namespace Unison.Baileys.Crypto
{
    /// <summary>
    ...
    public static class CryptoUtils
    {
        ...
        public static byte[] AesCbcDecrypt(byte[] ciphertext, byte[] key, byte[] iv)
        {
            ...
            return AesCbcDecrypt(ciphertext, 0, ciphertext.Length, key, iv);
        }

        public static byte[] AesCbcDecrypt(byte[] ciphertext, int offset, int count, byte[] key, byte[] iv)
        {
            if (ciphertext == null) throw new ArgumentNullException(nameof(ciphertext));
            if (offset < 0 || count < 0 || offset + count > ciphertext.Length)
            ...
            using (var aes = System.Security.Cryptography.Aes.Create())
            {
                ...
                using (var ms = new System.IO.MemoryStream(ciphertext, offset, count, false))
                using (var cs = new System.Security.Cryptography.CryptoStream(ms, decryptor, System.Security.Cryptography.CryptoStreamMode.Read))
                using (var output = new System.IO.MemoryStream(Math.Max(0, count)))
                {
                    cs.CopyTo(output);
        ...
        {
            using (var output = DecompressZlibToStream(data))
            {
                return output?.ToArray();
            }
        }
        ...
        /// <summary>
        /// Decompresses Zlib into a seekable stream. History sync uses this overload so
        /// parsing does not require a second full-size byte-array copy.
        /// </summary>
        public static MemoryStream DecompressZlibToStream(byte[] data)
        {
            ...
            try
            {
                using (var ms = new MemoryStream(data, 2, data.Length - 2, false))
        ...
        public static byte[] HmacSha256(byte[] data, byte[] key)
        {
            ...
            return HmacSha256(null, data, 0, data.Length, key);
        }

        public static byte[] HmacSha256(byte[] prefix, byte[] data, int offset, int count, byte[] key)
        {
            if (data == null) throw new ArgumentNullException(nameof(data));
            ...
            {
                hmac.BlockUpdate(prefix, 0, prefix.Length);
            }
        ...
        /// <summary>
        /// Computes HMAC-SHA256 over an optional prefix followed by exactly
        /// <paramref name="count"/> bytes from a stream. This avoids allocating a
        /// complete history-sync blob in managed memory.
        /// </summary>
        public static byte[] HmacSha256(Stream stream, long count, byte[] prefix, byte[] key)
        {
            ...
            while (remaining > 0)
            {
                ...
                if (read <= 0)
                {
                    throw new EndOfStreamException("Unexpected end of encrypted media stream.");
                }
