    ...
    public class BinaryDecoder
    {
        ...
        public static BinaryNode Decode(byte[] data)
        {
            ...
            if (isCompressed)
            {
                ...
                ProtocolRuntimeLog.Write($"[BinaryDecoder] Decompressed {data.Length - 1} -> {payload.Length} bytes");
            }
            else
            {
        ...
        private static byte[] DecompressZlib(byte[] data, int offset)
        {
            ...
            {
                ProtocolRuntimeLog.Write($"[BinaryDecoder] Zlib decompression failed: {ex.Message}");
        ...
        private string ReadString()
        {
            ...
            switch (b)
            {
                ...
                case 247: // AD_JID - advanced JID (domainType + device + user)
                    {
                        byte rawDomainType = ReadByte();
                        int domainType = rawDomainType;
                        byte device = ReadByte();
                        string user = ReadString();

                        // Baileys readAdJid: domainType maps to server suffix
                        string server = WA.S_WHATSAPP_NET;
                        if (domainType == 1)
                        {
                            server = "lid";
                        }
