...
namespace Unison.Baileys.Protocol
{
    ...
    public class NoiseHandler
    {
        ...
        public NoiseSessionState ExportState()
        {
            lock (_stateLock)
            {
                return new NoiseSessionState
                {
                    Hash = NoiseSessionState.CloneBytes(_hash),
                    Salt = NoiseSessionState.CloneBytes(_salt),
                    EncryptionKey = NoiseSessionState.CloneBytes(_encKey),
                    DecryptionKey = NoiseSessionState.CloneBytes(_decKey),
                    ReadCounter = _readCounter,
                    WriteCounter = _writeCounter,
                    IsFinished = _isFinished,
                    SentIntro = _sentIntro,
                    PendingInput = NoiseSessionState.CloneBytes(_inBytes)
                };
            }
        }

        public void ImportState(NoiseSessionState state)
        {
            if (state == null || !state.IsValidEstablishedState())
            {
                throw new ArgumentException("Invalid established Noise state", nameof(state));
            }

            lock (_stateLock)
            {
                _hash = NoiseSessionState.CloneBytes(state.Hash) ?? new byte[0];
                _salt = NoiseSessionState.CloneBytes(state.Salt) ?? new byte[0];
                _encKey = NoiseSessionState.CloneBytes(state.EncryptionKey);
                _decKey = NoiseSessionState.CloneBytes(state.DecryptionKey);
                _readCounter = state.ReadCounter;
                _writeCounter = state.WriteCounter;
                _isFinished = state.IsFinished;
                _sentIntro = state.SentIntro;
                _inBytes = NoiseSessionState.CloneBytes(state.PendingInput) ?? new byte[0];
            }
        }

        public NoiseHandler(KeyPair keyPair, byte[] routingInfo = null, IProtocolLogger logger = null)
        {
            _keyPair = keyPair;
            ...
            ProtocolRuntimeLog.Write($"[Noise] Initialized with public key: {BitConverter.ToString(_keyPair.Public).Replace("-", "").Substring(0, 16)}...");
        }
        ...
        public void Authenticate(byte[] data)
        {
            lock (_stateLock)
            {
                if (!_isFinished)
                {
                    ...
                    ProtocolRuntimeLog.Write($"[Noise] Authenticated {data.Length} bytes, hash: {BitConverter.ToString(_hash).Replace("-", "").Substring(0, 16)}...");
                }
            }
        }
        ...
        public byte[] Encrypt(byte[] plaintext)
        {
            lock (_stateLock)
            {
                ...
                ProtocolRuntimeLog.Write($"[Noise] Encrypted {plaintext.Length} bytes -> {result.Length} bytes, counter: {_writeCounter}");
                return result;
            }
        }
        ...
        public byte[] Decrypt(byte[] ciphertext)
        {
            lock (_stateLock)
            {
                ...
                ProtocolRuntimeLog.Write($"[Noise] Decrypted {ciphertext.Length} bytes -> {result.Length} bytes");
                return result;
            }
        }
        ...
        public void MixIntoKey(byte[] data)
        {
            lock (_stateLock)
            {
                ...
                _readCounter = 0;
                _writeCounter = 0;
                ProtocolRuntimeLog.Write($"[Noise] Mixed key, new encKey: {BitConverter.ToString(_encKey).Replace("-", "").Substring(0, 16)}...");
            }
        }
        ...
        public void FinishInit()
        {
            lock (_stateLock)
            {
                ...
                _writeCounter = 0;
                _isFinished = true;
                ProtocolRuntimeLog.Write("[Noise] Handshake finished, encrypted channel established");
            }
        }
        ...
        {
            ProtocolRuntimeLog.Write($"[Noise] Processing handshake...");
            ProtocolRuntimeLog.Write($"[Noise] Server ephemeral: {serverEphemeral.Length} bytes");
            ProtocolRuntimeLog.Write($"[Noise] Server static encrypted: {serverStaticEncrypted.Length} bytes");
            ProtocolRuntimeLog.Write($"[Noise] Server payload encrypted: {serverPayloadEncrypted.Length} bytes");

            // *** LOG NOISE KEYS FOR DEBUGGING ***
            _logger?.LogKeyInfo("Noise Handshake Inputs", new System.Collections.Generic.Dictionary<string, string>
            {
            ...
            _logger?.LogKeyInfo("After DH(ee)", new System.Collections.Generic.Dictionary<string, string>
            {
            ...
            ProtocolRuntimeLog.Write($"[Noise] Decrypted server static: {serverStatic.Length} bytes");
            
            // 4. DH with our ephemeral and server static
            var sharedES = CryptoUtils.SharedKey(_keyPair.Private, serverStatic);
            MixIntoKey(sharedES);
            
            _logger?.LogKeyInfo("After DH(es)", new System.Collections.Generic.Dictionary<string, string>
            {
            ...
            ProtocolRuntimeLog.Write($"[Noise] Decrypted cert payload: {certPayload.Length} bytes");
            ...
            ProtocolRuntimeLog.Write($"[Noise] Encrypted noise key: {keyEnc.Length} bytes");
            
            // 7. DH with noise key and server ephemeral
            var sharedSE = CryptoUtils.SharedKey(noiseKey.Private, serverEphemeral);
            MixIntoKey(sharedSE);
            
            _logger?.LogKeyInfo("After DH(se) - Final", new System.Collections.Generic.Dictionary<string, string>
            {
        ...
        public byte[] EncodeFrame(byte[] data)
        {
            ...
            ProtocolRuntimeLog.Write($"[Noise] Encoded frame: {frame.Length} bytes (data: {data.Length}, intro: {introSize})");
            return frame;
        }
        ...
        public async Task DecodeFrame(byte[] newData, Func<byte[], Task> onFrame)
        {
            if (newData != null)
            {
                lock (_stateLock)
                {
                    ...
                    _inBytes = combined;
                    ProtocolRuntimeLog.Write($"[Noise] Received {newData.Length} bytes, total buffer: {_inBytes.Length} bytes");
                }
            }
            ...
            while (true)
            {
                ...
                lock (_stateLock)
                {
                    ...
                    {
                        ProtocolRuntimeLog.Write($"[Noise] Waiting for more data, need {size + 3}, have {_inBytes.Length}");
                        return;
                    }
                ...
                {
                    ProtocolRuntimeLog.Write($"[Noise] Decoded frame: {frameToProcess.Length} bytes");
