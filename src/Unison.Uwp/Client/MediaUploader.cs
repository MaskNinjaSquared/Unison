...
using Unison.Baileys.Protocol; // For BinaryNode
...
namespace Unison.Uwp.Client
{
    public class MediaUploader
    {
        ...
        private const string MEDIA_PATH_AUDIO = "/mms/audio";
        ...
        public Task<MediaUploadResult> UploadImageAsync(byte[] imageBytes)
        {
            return UploadMediaAsync(imageBytes, "image", MEDIA_PATH_IMAGE, "image/jpeg");
        }

        public Task<MediaUploadResult> UploadAudioAsync(byte[] audioBytes, string mimeType)
        {
            return UploadMediaAsync(audioBytes, "audio", MEDIA_PATH_AUDIO,
                string.IsNullOrWhiteSpace(mimeType) ? "audio/mp4" : mimeType);
        }

        private async Task<MediaUploadResult> UploadMediaAsync(byte[] mediaBytes, string mediaType, string mediaPath, string mimeType)
        {
            if (mediaBytes == null || mediaBytes.Length == 0)
                throw new ArgumentException("Media payload is empty", nameof(mediaBytes));

            var encrypted = await MediaUtils.EncryptMediaAsync(mediaBytes, mediaType);
            ...
            foreach (var host in mediaConn.Hosts)
            {
                try
                {
                    string url = $"https://{host}{mediaPath}/{encSha256B64}?auth={authEncoded}&token={encSha256B64}";
