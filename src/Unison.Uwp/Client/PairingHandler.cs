...
namespace Unison.Uwp.Client
{
    ...
    /// </summary>
    public class PairingHandler : IPairingService
    {
        ...
        public async Task<string> RequestPairingCodeAsync(string phoneNumber, string customCode = null)
        {
            ...
            }, new List<BinaryNode>
            {
                ...
                }, new List<BinaryNode>
                {
                    ...
                    // Per Baileys macOS Desktop: companion_platform_id is ELECTRON ("7")
                    new BinaryNode("companion_platform_id", null, System.Text.Encoding.UTF8.GetBytes("7")),
                    new BinaryNode("companion_platform_display", null, System.Text.Encoding.UTF8.GetBytes("Desktop (Mac OS)")),
            ...
            try
            {
                ...
                if (response.Attrs.TryGetValue("type", out var type) && type == "error")
                {
                    ...
                    var errorCode = errorNode?.Attrs.GetDictionaryValueOrDefault("code", "unknown");
        ...
        /// <summary>
        /// Gets the platform ID for macOS Desktop (from Baileys getCompanionPlatformId)
        /// </summary>
        private byte[] GetPlatformId()
        {
            // Platform ID 7 = ELECTRON for browser tuple ['Mac OS', 'Desktop', '14.4.1']
            return new byte[] { 7 };
        }
        ...
        public async Task HandlePairSuccessAsync(BinaryNode stanza)
        {
            ...
            try
            {
                ...
                var jid = deviceNode?.Attrs.GetDictionaryValueOrDefault("jid", null);
                ...
                // Trust the server's JID as the primary identity
                string normalizedJid = WA.NormalizeDeviceJid(jid);
                bool isLid = normalizedJid.EndsWith("@lid");
                
                _socket.Auth.Me = new UserInfo
                {
                    Id = isLid ? (_socket.Auth.Me?.Id ?? normalizedJid) : normalizedJid,
                    Lid = isLid ? normalizedJid : (_socket.Auth.Me?.Lid),
                    Name = bizNode?.Attrs.GetDictionaryValueOrDefault("name", "~")
                };
                
                Debug.WriteLine($"[Pairing] Identity set from server JID: {_socket.Auth.Me.Id} (LID: {_socket.Auth.Me.Lid})");
                
                // If we only had one before (e.g. PN in Id) and now have another (e.g. LID in normalizedJid),
                // they are both now in the Me object.
                _socket.Auth.Registered = true;

                // Save state
                await _authStore.SaveAsync(_socket.Auth);
                
                // Also persist account to KeyStore for session sends
                if (_socket.KeyStore != null)
                {
                    await _socket.KeyStore.SetAccountAsync(_socket.Auth.Account);
                    Debug.WriteLine("[Pairing] Saved account to KeyStore");
                }
